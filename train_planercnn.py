...
def train(options):
    ...
    if options.restore == 1:
        ...
        model.load_state_dict(torch.load(
            options.checkpoint_dir + '/checkpoint.pth'))
        refine_model.load_state_dict(torch.load(
            options.checkpoint_dir + '/checkpoint_refine.pth'))
    ...
    if options.restore == 1 and os.path.exists(options.checkpoint_dir + '/optim.pth'):
        optimizer.load_state_dict(torch.load(
            options.checkpoint_dir + '/optim.pth'))
        pass
    ...
    for epoch in range(options.numEpochs):
        ...
        for sampleIndex, sample in enumerate(data_iterator):
            ...
            for indexOffset in [0, 13]:
                images, image_metas, rpn_match, rpn_bbox, gt_class_ids, gt_boxes, gt_masks, gt_parameters, gt_depth, extrinsics, gt_plane, gt_segmentation, plane_indices = sample[indexOffset + 0].cuda(), sample[indexOffset + 1].numpy(), sample[indexOffset + 2].cuda(), sample[indexOffset + 3].cuda(
                ...
                if indexOffset == 13:
                    input_pair.append({'image': images, 'depth': gt_depth, 'mask': gt_masks, 'bbox': gt_boxes,
                                       'extrinsics': extrinsics, 'segmentation': gt_segmentation, 'plane': gt_plane, 'camera': camera})
                    continue
                rpn_class_logits, rpn_pred_bbox, target_class_ids, mrcnn_class_logits, target_deltas, mrcnn_bbox, target_mask, mrcnn_mask, target_parameters, mrcnn_parameters, detections, detection_masks, detection_gt_parameters, detection_gt_masks, rpn_rois, roi_features, roi_indices, feature_map, depth_np_pred = model.predict(
                    [images, image_metas, gt_class_ids, gt_boxes, gt_masks, gt_parameters, camera], mode='training_detection', use_nms=2, use_refinement='refinement' in options.suffix, return_feature_map=True)

                rpn_class_loss, rpn_bbox_loss, mrcnn_class_loss, mrcnn_bbox_loss, mrcnn_mask_loss, mrcnn_parameter_loss = compute_losses(
                    config, rpn_match, rpn_bbox, rpn_class_logits, rpn_pred_bbox, target_class_ids, mrcnn_class_logits, target_deltas, mrcnn_bbox, target_mask, mrcnn_mask, target_parameters, mrcnn_parameters)

                losses += [rpn_class_loss + rpn_bbox_loss + mrcnn_class_loss +
                ...
                if config.PREDICT_NORMAL_NP:
                    ...
                    depth_np_loss = l1LossMask(
                        depth_np_pred[:, 80:560], gt_depth[:, 80:560], (gt_depth[:, 80:560] > 1e-4).float())
                    normal_np_loss = l2LossMask(normal_np_pred[:, 80:560], gt_normal[:, 80:560], (torch.norm(
                        gt_normal[:, 80:560], dim=0) > 1e-4).float())
                ...
                if len(detections) > 0:
                    detections, detection_masks = unmoldDetections(
                        config, camera, detections, detection_masks, depth_np_pred, normal_np_pred, debug=False)
                    if 'refine_only' in options.suffix:
                        detections, detection_masks = detections.detach(), detection_masks.detach()
                        pass
                    XYZ_pred, detection_mask, plane_XYZ = calcXYZModule(
                        config, camera, detections, detection_masks, depth_np_pred, return_individual=True)
                ...
                input_pair.append({'image': images, 'depth': gt_depth, 'mask': gt_masks, 'bbox': gt_boxes, 'extrinsics': extrinsics,
                                   'segmentation': gt_segmentation, 'parameters': detection_gt_parameters, 'plane': gt_plane, 'camera': camera})
                detection_pair.append({'XYZ': XYZ_pred, 'depth': XYZ_pred[1:2], 'mask': detection_mask, 'detection': detections,
                                       'masks': detection_masks, 'feature_map': feature_map[0], 'plane_XYZ': plane_XYZ, 'depth_np': depth_np_pred})

                if 'depth' in options.suffix:
                    # Apply supervision on reconstructed depthmap (not used currently)
                    if len(detections) > 0:
                        background_mask = torch.clamp(
                            1 - detection_masks.sum(0, keepdim=True), min=0)
                        all_masks = torch.cat(
                            [background_mask, detection_masks], dim=0)

                        all_masks = all_masks / all_masks.sum(0, keepdim=True)
                        all_depths = torch.cat(
                            [depth_np_pred, plane_XYZ[:, 1]], dim=0)

                        depth_loss = l1LossMask(torch.sum(torch.abs(all_depths[:, 80:560] - gt_depth[:, 80:560]) * all_masks[:, 80:560], dim=0), torch.zeros(
                            config.IMAGE_MIN_DIM, config.IMAGE_MAX_DIM).cuda(), (gt_depth[0, 80:560] > 1e-4).float())
                    else:
            ...
            if (len(detection_pair[0]['detection']) > 0 and len(detection_pair[0]['detection']) < 30) and 'refine' in options.suffix:
                ...
                image = (
                    input_dict['image'] + config.MEAN_PIXEL_TENSOR.view((-1, 1, 1))) / 255.0 - 0.5
                image_2 = (
                    input_pair[1 - c]['image'] + config.MEAN_PIXEL_TENSOR.view((-1, 1, 1))) / 255.0 - 0.5
                ...
                if 'large' not in options.suffix:
                    ...
                    plane_depth = torch.nn.functional.interpolate(plane_depth[:, 80:560].unsqueeze(
                        1).float(), size=(192, 256), mode='bilinear').squeeze(1)
                ...
                segmentation_one_hot = (segmentation == torch.arange(
                    segmentation.max() + 1).cuda().view((-1, 1, 1, 1))).long()
                intersection = (torch.round(detection_masks).long()
                                * segmentation_one_hot).sum(-1).sum(-1)
                max_intersection, segments_gt = intersection.max(0)
                mapping = intersection.max(1)[1]
                detection_areas = detection_masks.sum(-1).sum(-1)
                valid_mask = (mapping[segments_gt] == torch.arange(
                    len(segments_gt)).cuda()).float()

                masks_gt_large = (
                    segmentation == segments_gt.view((-1, 1, 1))).float()
                masks_gt_small = masks_gt_large[:, ::4, ::4]
                planes_gt = input_dict['plane'][0][segments_gt]
                # Run the refinement network
                results = refine_model(image, image_2, camera, masks_inp,
                                       detection_dict['detection'][:, 6:9], plane_depth, depth_np)
                ...
                for resultIndex, result in enumerate(results[1:]):
                    if 'mask' in result:
                        ...
                        masks_logits = masks_pred.squeeze(1).transpose(0, 1).transpose(
                            1, 2).contiguous().view((segmentation.shape[0], -1))
                        detection_areas = all_masks_gt.sum(-1).sum(-1)
                        detection_weight = detection_areas / detection_areas.sum()
                        detection_weight = - \
                            torch.log(torch.clamp(
                                detection_weight, min=1e-4, max=1 - 1e-4))
                        if 'weight' in options.suffix:
                            mask_loss += torch.nn.functional.cross_entropy(
                                masks_logits, segmentation, weight=detection_weight)
                        else:
                            mask_loss += torch.nn.functional.cross_entropy(
                                masks_logits, segmentation, weight=torch.cat([torch.ones(1).cuda(), valid_mask], dim=0))
                            pass
                        masks_pred = (masks_pred.max(0, keepdim=True)[1] == torch.arange(
                            len(masks_pred)).cuda().long().view((-1, 1, 1))).float()[1:]
                        pass
                    continue
                losses += [mask_loss + depth_loss +
                           plane_depth_loss + plane_loss]
                ...
                if 'refine_after' in options.suffix:
                    ...
                    XYZ_pred, detection_mask, plane_XYZ = calcXYZModule(
                        config, camera, detections, detection_masks, detection_dict['depth_np'], return_individual=True)
            ...
            for c in range(1, 2):
                ...
                neighbor_info = torch.cat(
                    [detection_dict['XYZ'], detection_dict['mask'], input_pair[1 - c]['image'][0]], dim=0).unsqueeze(0)
                warped_info, valid_mask = warpModuleDepth(config, camera, input_pair[c]['depth'][0], neighbor_info, input_pair[c][
                                                          'extrinsics'][0], input_pair[1 - c]['extrinsics'][0], width=config.IMAGE_MAX_DIM, height=config.IMAGE_MIN_DIM)

                XYZ = warped_info[:3].view((3, -1))
                XYZ = torch.cat(
                    [XYZ, torch.ones((1, int(XYZ.shape[1]))).cuda()], dim=0)
                transformed_XYZ = torch.matmul(input_pair[c]['extrinsics'][0], torch.matmul(
                    input_pair[1 - c]['extrinsics'][0].inverse(), XYZ))
                transformed_XYZ = transformed_XYZ[:3].view(
                    detection_dict['XYZ'].shape)
            ...
            if sampleIndex % 500 < options.batchSize or options.visualizeMode == 'debug':
                ...
                visualizeBatchPair(options, config, input_pair,
                                   detection_pair, indexOffset=sampleIndex % 500)
                if (len(detection_pair[0]['detection']) > 0 and len(detection_pair[0]['detection']) < 30) and 'refine' in options.suffix:
                    visualizeBatchRefinement(options, config, input_pair[0], [
                                             {'mask': masks_gt, 'plane': planes_gt}, ] + results, indexOffset=sampleIndex % 500, concise=True)
                    pass
            ...
            if (sampleIndex + 1) % options.numTrainingImages == 0:
                ...
                torch.save(model.state_dict(),
                           options.checkpoint_dir + '/checkpoint.pth')
                torch.save(refine_model.state_dict(),
                           options.checkpoint_dir + '/checkpoint_refine.pth')
...
if __name__ == '__main__':
    ...
    if False:
        writeHTML(args.test_dir, ['image_0', 'segmentation_0', 'depth_0', 'depth_0_detection', 'depth_0_detection_ori'], labels=[
                  'input', 'segmentation', 'gt', 'before', 'after'], numImages=20, image_width=160, convertToImage=True)
        exit(1)
