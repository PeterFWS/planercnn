...
def evaluateDepths(predDepths, gtDepths, printInfo=False):
    ...
    rmse_log = np.sqrt((pow(np.log(np.maximum(predDepths, 1e-4)) -
                            np.log(np.maximum(gtDepths, 1e-4)), 2) * masks).sum() / numPixels)
    ...
    if printInfo:
        print(('depth statistics', rel, rel_sqr, log10, rmse,
               rmse_log, accuracy_1, accuracy_2, accuracy_3))
        pass
...
def evaluatePlanesTensor(input_dict, detection_dict, printInfo=False, use_gpu=True):
    ...
    if use_gpu:
        masks_pred, masks_gt, depth_pred, depth_gt = detection_dict[
            'masks'], input_dict['masks'], detection_dict['depth'], input_dict['depth']
    else:
        masks_pred, masks_gt, depth_pred, depth_gt = detection_dict['masks'].cpu(
        ), input_dict['masks'].cpu(), detection_dict['depth'].cpu(), input_dict['depth'].cpu()
        pass

    masks_pred = torch.round(masks_pred)

    plane_areas = masks_gt.sum(dim=1).sum(dim=1)
    masks_intersection = (masks_gt.unsqueeze(
        1) * (masks_pred.unsqueeze(0))).float()
    intersection_areas = masks_intersection.sum(2).sum(2)

    depth_diff = torch.abs(depth_gt - depth_pred)
    depth_diff[depth_gt < 1e-4] = 0

    depths_diff = (depth_diff * masks_intersection).sum(2).sum(2) / \
        torch.clamp(intersection_areas, min=1e-4)
    depths_diff[intersection_areas < 1e-4] = 1000000

    union = ((masks_gt.unsqueeze(1) + masks_pred.unsqueeze(0))
             > 0.5).float().sum(2).sum(2)
    ...
    for IOU_threshold in [0.5, ]:
        ...
        min_diff = np.min(depths_diff * IOU_mask +
                          1e6 * (1 - IOU_mask), axis=1)
        stride = 0.05
        plane_recall = []
        pixel_recall = []
        for step in range(21):
            diff_threshold = step * stride
            pixel_recall.append(np.minimum((intersection_areas * ((depths_diff <= diff_threshold).astype(
                np.float32) * IOU_mask)).sum(1), plane_areas).sum() / num_plane_pixels)
    ...
    for diff_threshold in [0.2, 0.3, 0.6, 0.9]:
        correct_mask = np.minimum(
            (depths_diff < diff_threshold), (plane_IOUs > 0.5))
    ...
    if printInfo:
        print('plane statistics', correct_mask.max(-1).sum(),
...
def evaluatePlaneDepth(config, input_dict, detection_dict, printInfo=False):
    ...
    ranges = config.getRanges(
        input_dict['camera']).transpose(1, 2).transpose(0, 1)
    ...
    for depth in [depth_pred, depth_gt]:
        ...
        plane_parameters = torch.stack([torch.matmul(torch.inverse(
            AA[planeIndex]), Ab[planeIndex]) for planeIndex in range(len(AA))], dim=0)
    ...
    statistics = [
        plane_diff.mean(), (plane_diff * plane_areas).sum() / plane_areas.sum()]
    if printInfo:
...
def evaluateMask(predMasks, gtMasks, printInfo=False):
    ...
    info = [intersection / max(predMasks.sum(), 1),
...
def evaluateMasksTensor(predMasks, gtMasks, valid_mask, printInfo=False):
    gtMasks = torch.cat([gtMasks, torch.clamp(
        1 - gtMasks.sum(0, keepdim=True), min=0)], dim=0)
    predMasks = torch.cat([predMasks, torch.clamp(
        1 - predMasks.sum(0, keepdim=True), min=0)], dim=0)
    intersection = (gtMasks.unsqueeze(1) * predMasks *
                    valid_mask).sum(-1).sum(-1).float()
    union = (torch.max(gtMasks.unsqueeze(1), predMasks)
             * valid_mask).sum(-1).sum(-1).float()

    N = intersection.sum()

    RI = 1 - ((intersection.sum(0).pow(2).sum() + intersection.sum(1).pow(2).sum()
               ) / 2 - intersection.pow(2).sum()) / (N * (N - 1) / 2)
    ...
    log2_quotient = torch.log2(torch.clamp(
        joint, 1e-8) / torch.clamp(B, 1e-8)) * (torch.min(joint, B) > 1e-8).float()
...
def evaluateBatchDeMoN(options, gt_dict, pred_dict, statistics=[[], [], []]):
    for batchIndex in range(len(gt_dict['depth'])):
        statistics[0].append(evaluateDepthRelative(pred_dict['depth'][batchIndex],
                                                   gt_dict['depth'][batchIndex], np.linalg.norm(gt_dict['translation'][batchIndex])))
        statistics[1].append(evaluateRotation(
            pred_dict['rotation'][batchIndex], gt_dict['rotation'][batchIndex]))
        statistics[2].append(evaluateTranslation(
...
def evaluateBatchDetection(options, config, input_dict, detection_dict, statistics=[[], [], [], []], debug_dict={}, printInfo=False, evaluate_plane=False):
    ...
    for c, plane_mask in enumerate([nyu_mask]):
        ...
        if padding > 0:
            valid_mask_depth = valid_mask_depth[:, 80 +
                                                padding:560 - padding, padding:-padding]
        else:
        ...
        if options.debug:
            print('\nmask', c)
            pass
        depth_statistics = evaluateDepths(depth_pred[valid_mask_depth].detach().cpu(
        ).numpy(), depth_gt[valid_mask_depth].detach().cpu().numpy(), printInfo=printInfo)
    ...
    if options.debug:
        if 'depth_np' in detection_dict:
            ...
            if padding > 0:
                depth_pred = depth_pred[:, 80 +
                                        padding:560 - padding, padding:-padding]
            else:
                depth_pred = depth_pred[:, 80:560]
                pass
            print('\nnon planar')
            evaluateDepths(depth_pred[valid_mask_depth].detach().cpu().numpy(
            ), depth_gt[valid_mask_depth].detach().cpu().numpy(), printInfo=True)
            pass
    ...
    statistics[2].append(evaluateMasksTensor(torch.round(detection_dict['masks']).cpu(
    ), input_dict['masks'].float().cpu(), valid_mask.float().cpu(), printInfo=printInfo))
...
def printStatisticsDetection(options, statistics):
    ...
    with open('logs/global.txt', open_type) as f:
        values = np.array(statistics[0]).mean(0).tolist(
        ) + np.array(statistics[1]).mean(0).tolist() + np.array(statistics[2]).mean(0).tolist()
...
def plotCurves(filename='test/curves.png', xlabel='depth threshold', ylabel='per plane recall %', title='', methods=['manhattan_pred', 'manhattan_gt', 'planenet_normal', 'refine_normal_refine']):
    ...
    final_labels = ['Manhattan + inferred depth',
                    'Manhattan + gt depth', 'PlaneNet', 'Ours']
        ...
        for line in f:
            ...
            if len(tokens) > max(cols) and method in methods and tokens[0].strip()[:-1] == dataset:
                method_statistics[method] = [
                    float(tokens[c].strip()) for c in cols]
                pass
            continue
        pass
